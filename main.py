...
max_seq_len = 350#TODO:change later when uses full document words
...
#        batch_outputs = []
    ...
    #                batch_outputs.append(y)
                    ...
                    #print('doc', i)
                print('score pas======', pas, ':', score(sess, prediction, input_vecs, test_data))
...
def run2():
    ...
    for doc in train_data:
        vecs = get_input_words_vec(doc.words)
        init = cell.zero_state(lstm_size, tf.float32)
        #lst_vec = vecs.tolist()
        pdb.set_trace()
        #outputs, last_state = tf.nn.rnn(cell, lst_vec)
